...
vel = Mesh.AVG1D*(q(:,:,2)./q(:,:,1));

switch Limit.ind_var
    case 'density'
        ind = Find_Tcells1D(q_ext(:,:,1),vel,Mesh,Limit,Net);
    case 'velocity'
        ind = Find_Tcells1D(q_ext(:,:,2)./q_ext(:,:,1),vel,Mesh,Limit,Net);
    case 'pressure'
        pre = (Problem.gas_gamma-1)*(q_ext(:,:,3) - 0.5*q_ext(:,:,2).^2./q_ext(:,:,1)); 
        ind = Find_Tcells1D(pre,vel,Mesh,Limit,Net);  
    case 'prim'
        pre = (Problem.gas_gamma-1)*(q_ext(:,:,3) - 0.5*q_ext(:,:,2).^2./q_ext(:,:,1)); 
        ind_d = Find_Tcells1D(q_ext(:,:,1),vel,Mesh,Limit,Net);
        ind_v = Find_Tcells1D(q_ext(:,:,2)./q_ext(:,:,1),vel,Mesh,Limit,Net);
        ind_p = Find_Tcells1D(pre,vel,Mesh,Limit,Net);
        ind   = unique([ind_d,ind_v,ind_p]);
    case 'con'
        ind_d = Find_Tcells1D(q_ext(:,:,1),vel,Mesh,Limit,Net);
        ind_m = Find_Tcells1D(q_ext(:,:,2),vel,Mesh,Limit,Net);
        ind_e = Find_Tcells1D(q_ext(:,:,3),vel,Mesh,Limit,Net);
